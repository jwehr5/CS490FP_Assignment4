...
(define (find-files-with-matches list-of-hashes  list-of-search-words)
  ...
  (define (count-matches current-hash current-filename list-of-search-words match-count)
    (cond
      [(and (empty? list-of-search-words) (> match-count 0))   (cons current-filename match-count)]
      [(and (empty? list-of-search-words) (= match-count 0))  empty]
  ...
  (if (empty? list-of-hashes)
      list-of-hashes
      (cons  (count-matches (first (hash-values (first list-of-hashes))) (first (hash-keys (first list-of-hashes))) list-of-search-words 0) (find-files-with-matches (rest list-of-hashes) list-of-search-words))))
...
;Main Program
(displayln "Hash is loading please wait...")
...
(define user-input (string-split (first (list (read-line (current-input-port) 'any)))))
